...
for i in $thermal_program ; do
	...
	if [ -f "$which_thermal" ]; then
		...
		if [ ! -n "$pgrep_thermal" ]; then
			echo "$which_thermal该温控进程异常没有启动，可能会无法快充，可能被系统屏蔽或被其它模块屏蔽，请恢复"
		fi
...
echo "当前模式$mode 温控档位$thermal_scene 充电场景$thermal_charge 游戏场景$thermal_app 刷新率$fps 充电$dumpsys_charging 旁辅$current_max 当前电流$current_now 旁停$stop_level 手动旁路$bypass_supply_mode 电量$battery_level 电量旁路$bypass_supply_level 温度$battery_temp 温度旁路$bypass_supply_temp 游戏旁路$bypass_supply_app"
echo --------- 电流节点 ----------
